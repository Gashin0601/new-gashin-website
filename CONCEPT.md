...
### ヒーローセクション（白黒→カラー演出）
**演出フロー**
1. **白黒のヒーローセクション**が表示される
   - ポートレート写真：グレースケール
   - テキスト：通常表示
2. ページ読み込み完了後、**約2秒で白黒からカラーへフェード**
3. 鮮やかなカラーのヒーローセクションに

**技術的考慮**
- CSS `filter: grayscale(100%)` → `grayscale(0%)` のトランジション
- `prefers-reduced-motion`では即座にカラー表示
- アニメーション完了後は通常のカラー状態を維持
...
**ハンバーガーメニュー機能**
- **音声読み上げのオン/オフ切り替え**
  - トグルスイッチで音声ナレーションの有効/無効を切り替え
  - 設定はlocalStorageに保存（永続化）
  - 初期値：スクリーンリーダー検出時は「オン」、それ以外は「オフ」
- **カラーテーマ切り替え**
  - ライトテーマ / ダークテーマの選択
  - 設定はlocalStorageに保存（永続化）
  - 初期値：システム設定（`prefers-color-scheme`）に従う
  - **適用範囲**：ストーリーページ以外のすべてのページ
    - ストーリーページは常に黒テーマ（シネマティック演出のため）
- ナビゲーションリンク（トップ / ストーリー / ニュース）
...
**ストーリーページ以外の全ページ**：ライトテーマ / ダークテーマ切り替え可能
- **ライトテーマ**（デフォルト）
  - 背景：`#ffffff`
  - テキスト：`#0b0c10`
  - セカンダリ背景：`#f5f5f5`
...
- **ダークテーマ**
  ...
  - セカンダリ背景：`#1a1b1f`
  - ボーダー：`#2a2b2f`
- **共通アクセント**：`#222284`（鈴木我信ブランドカラー）

**ストーリーページ**：黒テーマ固定（シネマティック演出）
- 背景：`#0b0c10`
- テキスト：`#ffffff`
- テーマ切り替え無効
...
- **CSS Variables + data属性**（テーマ切替）
  - `<html data-theme="light">` / `<html data-theme="dark">`
  - CSS Variables: `--bg-primary`, `--text-primary`, `--bg-secondary`, `--border-color`
  - localStorage + `prefers-color-scheme` 自動検出
  - ストーリーページは `data-theme="story"` で固定（黒テーマ）
...
- 状態をlocalStorageに保存
- トグル時に即座に反映
- スクリーンリーダーには状態を通知（"音声読み上げをオンにしました"）

**テーマ切り替え**
- ラジオボタングループまたはセグメントコントロール
- `role="radiogroup"` with `aria-label="カラーテーマ"`
- 選択肢："ライト" / "ダーク" / "システム設定"
- 変更時にlocalStorageに保存、即座にテーマ適用
- ストーリーページでは非表示または無効化
...
[
  ...
  },
  {
    ...
    "title": "J-CASTニュースさんにて、ローソンでの撮影禁止についてXの発信が話題に",
...
- [ ] ハンバーガーメニューのA11y完全対応
- [ ] ハンバーガーメニュー機能実装
  - [ ] 音声読み上げのオン/オフトグル（localStorage永続化）
  - [ ] カラーテーマ切り替え（ライト/ダーク/システム設定）
  - [ ] ストーリーページではテーマ切り替え無効
